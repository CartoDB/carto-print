...
class Printer(object):
    ...
    def prepare_url(self, tile_size, lon, lat, zoom):
        ...
        return (self.server_url + '/api/v1/map/static/center/{template}/{zoom}/{lat}/{lon}/{tile_size}/{tile_size}.png?api_key={api_key}').format(username=self.username, template=self.map_id, tile_size=tile_size, zoom=zoom, lat=lat, lon=lon, api_key=self.api_key)
